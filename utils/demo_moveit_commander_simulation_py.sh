...
sleep 5
byobu send-keys -t 2 '(xhost +local:root >/dev/null 2>&1 || true); docker exec -it iiwa_ros2_container bash -it -c "ros2 launch lbr_moveit_py hello_moveit_commander.launch.py mode:=mock model:=iiwa14"' 'C-m'
