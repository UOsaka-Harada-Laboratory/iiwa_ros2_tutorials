...
byobu send-keys -t 0 'xhost + && docker exec -it iiwa_ros2_container bash -it -c "cp --force /yaml/lbr_controllers_position_command_controller.yaml /root/ros2_ws/src/lbr-stack/src/lbr_fri_ros2_stack/lbr_description/ros2_control/lbr_controllers.yaml"' 'C-m'
sleep 1.
byobu send-keys -t 0 'xhost + && docker exec -it iiwa_ros2_container bash -it -c "ros2 launch lbr_bringup mock.launch.py ctrl:=lbr_joint_position_command_controller model:=iiwa14"' 'C-m'
sleep 8.
...
sleep 5.
byobu send-keys -t 2 'xhost + && docker exec -it iiwa_ros2_container bash -it -c "python3 /root/joint_sine_overlay_sim.py --ros-args -r __ns:=/lbr"' 'C-m'
