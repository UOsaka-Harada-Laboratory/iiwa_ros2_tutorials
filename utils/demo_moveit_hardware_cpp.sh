...
sleep 1.
byobu send-keys -t 0 'xhost + && docker exec -it iiwa_ros2_container bash -it -c "ros2 launch lbr_bringup hardware.launch.py moveit:=true model:=iiwa14"' 'C-m'
sleep 2.
...
sleep 2.
byobu send-keys -t 2 'xhost + && docker exec -it iiwa_ros2_container bash -it -c "ros2 launch lbr_moveit_cpp hello_moveit.launch.py mode:=hardware model:=iiwa14"' 'C-m'
