...
byobu send-keys -t 0 "(xhost +local:root >/dev/null 2>&1 || true); docker exec -e ROS_DOMAIN_ID=${IIWA_ROS_DOMAIN_ID} -it iiwa_ros2_container bash -it -c \"ros2 launch lbr_bringup mock.launch.py model:=iiwa14\"" 'C-m'
sleep 2
byobu send-keys -t 1 "(xhost +local:root >/dev/null 2>&1 || true); docker exec -e ROS_DOMAIN_ID=${IIWA_ROS_DOMAIN_ID} -it iiwa_ros2_container bash -it -c \"ros2 launch lbr_bringup rviz.launch.py rviz_cfg_pkg:=lbr_bringup rviz_cfg:=config/mock.rviz\"" 'C-m'
