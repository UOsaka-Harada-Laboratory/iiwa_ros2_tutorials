...
byobu new-session -d -s demo_moveit_simulation_py
...
sleep 5.
byobu send-keys -t 2 'xhost + && docker exec -it iiwa_ros2_container bash -it -c "ros2 launch lbr_moveit_py hello_moveit.launch.py mode:=mock model:=iiwa14"' 'C-m'

byobu attach -t demo_moveit_simulation_py